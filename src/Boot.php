...
class Boot extends Base {
    ...
    public function PushMsg(array $array){
        !isset($this->msg_type[$array['type']]) && $this->datamow->datamsg(self::LOSE,'请填写正确的消息类型');
        (empty($array['content']) || $this->datamow->analyJson($array['content'])==false) && $this->datamow->datamsg(self::LOSE,'请填写正确格式的消息内容');
        $data = [
